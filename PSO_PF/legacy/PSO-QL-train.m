...
dFVg =[];
real_fitness = 1;
ger1 = 50;% 初始位置预测pso迭代次数

for rpE = 1:rp_time
for stateE = 1 : free_spot_N
    if mod(stateE,5)==0 || stateE == free_spot_N
        stateE
    end
...
%             
    ...
    dFR = fitness_var(fitness_var >= fitness_var_threshold);%淘汰位态fitness变化量和
    ElR = eliminate_potential_pos_N / potential_pos_N;
    if numel(dFR) == 0
        sum_dFR = 0;
        dFV =  0;
    else
%                 mean_dFR = mean(dFR) / fitness_var_threshold;
        ...
        dFV =  var(dFR);%淘汰位态fitness变化量方差
    end
    reward = ElRw*ElR+ sum_dFRw*sum_dFR - dFVw*dFV;% reward公式
